...
// Text colours (ARGB) — no fill, just font.
const TEXT_GREEN = 'FF1B7A3E';   // deep green
const TEXT_AMBER = 'FFB07A1F';   // gold/amber
const TEXT_RED   = 'FFB0282C';   // muted red
const HEADER_FILL = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1F3A6E' } };
const HEADER_FONT = { bold: true, color: { argb: 'FFFFFFFF' } };
const SUBHEADER_FILL = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFEAEEF6' } };
const SUBHEADER_FONT = { bold: true, color: { argb: 'FF1F3A6E' } };
...
function textColorFor(category) {
  if (category === 'green') return TEXT_GREEN;
  if (category === 'amber') return TEXT_AMBER;
  return TEXT_RED;
}
...
async function buildWorkbook(rows) {
  ...
  // ── Colour key (rendered to the right of the header + first 4 rows) ──────
  ...
  // Three coloured key rows (2-4): label in P, description in Q.
  const keyEntries = [
    { color: TEXT_GREEN, label: 'GREEN', desc: 'Till took payment + Shopify paid + fulfilled' },
    { color: TEXT_AMBER, label: 'AMBER', desc: 'Partial — one or two of the three signals' },
    { color: TEXT_RED,   label: 'RED',   desc: 'No movement — not paid, not fulfilled' }
  ];
  ...
  for (const r of rows) {
    ...
    const argb = textColorFor(category);
    // Apply text colour to the data columns only (1-14), leave key area clean.
    for (let i = 1; i <= 14; i++) {
      row.getCell(i).font = { color: { argb } };
    }
    row.getCell('category').font = { bold: true, color: { argb } };
  }
  ...
  const summaryRows = [
    ...
    ['Green (paid + fulfilled)',  greenCount, TEXT_GREEN],
    ['Amber (partial)',           amberCount, TEXT_AMBER],
    ['Red (no movement)',         redCount,   TEXT_RED],
